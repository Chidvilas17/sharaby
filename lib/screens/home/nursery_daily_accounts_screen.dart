...
class _NurseryDailyAccountsScreenState
  ...
  Future<void> _search() async {
    ...
    try {
      ...
      _showMessage(
        'Accounts loaded for ${_formatDate(selectedDate)}.',
      );
    } catch (e) {
      if (!mounted) return;

      _showMessage(
        'Failed to load nursery accounts: $e',
      );
    } finally {
      if (!mounted) return;
  ...
  Future<void> _deleteIncome() async {
    ...
    if (selectedIncomeRow! < 0 ||
        selectedIncomeRow! >= incomeRecords.length) {
      _showMessage(
    ...
    final record =
    incomeRecords[selectedIncomeRow!];
  ...
  Future<void> _deleteExpense() async {
    ...
    if (selectedExpenseRow! < 0 ||
        selectedExpenseRow! >= expenseRecords.length) {
      _showMessage(
    ...
    final record =
    expenseRecords[selectedExpenseRow!];
  ...
  Widget _buildTable({
    ...
    final records =
    income ? incomeRecords : expenseRecords;
      ...
      child: Table(
        ...
        children: [
          ...
          ...List.generate(15, (index) {
            ...
            final record =
            hasData ? records[index] : null;

            String valueForHeader(
                String header,
                ) {
              if (record == null) {
                return '';
  ...
  Widget _buildAccountSection({
    ...
    return Column(
      ...
      children: [
        ...
        Text(
          income
              ? 'Total Income: ${_calculateTotal(incomeRecords)}'
              : 'Total Expenses: ${_calculateTotal(expenseRecords)}',
          textAlign: TextAlign.center,
          style: const TextStyle(
            color: Colors.blue,
            fontSize: 20,
